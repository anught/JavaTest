...
public class StreamDemo {
	...
	static void gen1() {
		...
		// strs1 = strs1.filter((x) -> x.equals("a"));
	...
	// generate
	static void gen3() {
		Stream<Integer> generate = Stream.generate(() -> 1);// 如果无limit 会无限的产生1
		generate.limit(10).forEach(System.out::println);
	}

	// 使用iterator
	static void gen4() {
		Stream<Integer> iterate = Stream.iterate(1, x -> x + 1); // 返回迭代元素 n-1 位置的值觉得n位置的值 ，初始值由种子决定
...
//        gen5();
		...
		Arrays.asList("java", "c#", "python", "scala").stream().sorted((a, b) -> a.length() - b.length())
		...
		Function<String, Boolean> isNumber = (str) -> {
		...
		String str = "11,22,33,4a,55";

		System.out.println(
				Stream.of(str.split(",")).filter(x -> isNumber.apply(x)).mapToInt(x -> Integer.valueOf(x)).sum());

		System.out.println(Stream.of(str.split(",")).filter(x -> isNumber.apply(x)).mapToInt(Integer::valueOf).sum());
		System.out.println(Stream.of(str.split(",")).filter(x -> isNumber.apply(x)).map(x -> Integer.valueOf(x))
				.mapToInt(x -> x).sum());
		System.out.println(
				Stream.of(str.split(",")).filter(x -> isNumber.apply(x)).map(Integer::valueOf).mapToInt(x -> x).sum());
		...
		System.out.println(Stream.of(str.split(",")).peek(System.out::println).filter(x -> isNumber.apply(x))
